...
class sub_routines(variables):
    ...
    def star(self):
        ...
        for K in range(self.NK):
            ...
            if self.F1[J]:
                ...
                self.USTAR1[K] = 0.5 * ((self.F1[I] / (self.A[I] * self.RHO)) + (self.F1[J] / (self.A[J] * self.RHO)))
            elif self.F1[I]:
    ...
    def wprim(self):
        ...
        for K in range(self.NK):
            ...
            if 0 <= I < len(self.IC) and 0 <= J < len(self.JC):
                ...
                AHDIA = 0.5 * (self.HDIA[I] + self.HDIA[J])                                                             ### avg hdia of adjacent subchannel
                AVRE = 0.5 * ((self.F1[I] / self.A[I]) + (self.F1[J] / self.A[J])) * AHDIA / self.VISC                                 ## avg re of adjacent subchannel
                #print(AVRE)
                BETA = 0.0018 * (AVRE ** (-0.1)) * (AHDIA / self.GAP[K]) * ((self.GAP[K] / self.RDIA) ** (1 - 0.4))          ### correlation for beta taken from finding
            
                print("AVRE:", AVRE, "AHDIA:", AHDIA, "GAP[K]:", self.GAP[K], "RDIA:", self.RDIA)

                self.WPR = BETA * self.GAP[K] * AVRE * self.VISC / AHDIA                                                       ## wpr is turbulent cross flow
                #print(WPR)
            else:
        ...
        for K in range(self.NK):
            I = self.IC[K] - 1
            J = self.JC[K] - 1
            if 0 <= I < len(self.IC) and 0 <= J < len(self.JC) and 0 <= K < len(self.XZ):
                #print(len(XZ))
                self.XZ[K] = ((self.F1[I] / self.A[I]) - (self.F1[J] / self.A[J])) * self.WPR / self.RHO
                #print(XZ[K])
            else:
        ...
        for I in range(self.NCHANL):
            SUM = 0.0
            for K in range(self.NK):
                if 0 <= I < len(self.ST) and 0 <= K < len(self.ST[I]) and 0 <= K < len(self.XZ):
    ...
    def xxa(self):
        ...
        for I in range(0, self.NCHANL):
            self.RE[I] = self.F0[I] * self.HDIA[I] / (self.A[I] * self.VISC)                            ## calculation of reynold no
            FACF[I] = 0.05052 * self.RE[I] ** (-0.05719)                            ## calculation of friction factor
            X4 = self.XY[I]
            X1 = (self.F0[I] / self.A[I]) ** 2                                             ## ratio of mass flow rate to area
            X2 = 0.5 * FACF[I] / (self.HDIA[I] * self.RHO)                                ## pressure drop  due to friction
            X3 = self.GC * self.RHO * math.cos(self.ALPHA * math.pi / 180)                      ## role of gravity
    ...
    def XD(self):
        ...
        for K in range(0, self.NK):
            if K < len(self.IC) and K < len(self.JC):
                I = self.IC[K] - 1
                J = self.JC[K] - 1
                self.CIJ0[K] = 0.5 * self.FACK * abs(self.WIJ0[K]) / (self.RHO * self.GAP[K] ** 2)
                self.CIJ1[K] = 0.5 * self.FACK * abs(self.WIJ1[K]) / (self.RHO * self.GAP[K] ** 2)
                self.D[K] = (self.USTAR1[K] / self.DELX) + (self.SLP * self.THETA * self.CIJ1[K])
        ...
        for K in range(self.NK):
            print(f'CIJ0[{K}] = {self.CIJ0[K]}, CIJ1[{K}] = {self.CIJ1[K]}, D[{K}] = {self.D[K]}, USTD1[{K}] = {self.USTD1[K]}')

    def XB(self):
        #Calls ski before
        print("subroutine XB")
        self.XMLT = self.XMULT(self.ST, self.XUSTD1, self.XMLT, self.NCHANL, self.NK, self.NK)
        ...
        for I in range(0, self.NCHANL):
            ...
            self.B2[I] = 2.0 * (self.F1[I ] - self.F0[I]) * (self.F1[I] / self.A[I] / self.RHO) / self.A[I]
            self.B[I] = self.DELX * self.XA[I] - self.B1[I] - self.B2[I]

        # Print the results
        for I in range(0, self.NCHANL):
            print(f"B1[{I}] = {self.B1[I ]}, B2[{I}] = {self.B2[I]}, B[{I}] = {self.B[I]}")    
    ...
    def AXIMOM(self):
        ...
        XU1 = [[U1[I] if I == J else 0.0 for J in range(self.NCHANL)] for I in range(self.NCHANL)]
    
        self.YU1 = [[0.0] * self.NK for _ in range(self.NCHANL)]
        self.YU1 = self.YMULT(self.XU1, self.ST, self.YU1, self.NCHANL, self.NCHANL, self.NK)
        ...
        self.P1 = [self.P0[I] + self.DELX * self.XA[I] + Z1[I] - Z2[I] for I in range(self.NCHANL)]
